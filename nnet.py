...
from settings import MUTATION_WEIGHT_MODIFY_CHANCE, MUTATION_ARRAY_MIX_PERC, train, lvl, load_weights, dw
...
class NNet:
    def __init__(self, num_input, num_hidden, num_output):
        ...
        if train and not load_weights:
            self.weight_input_hidden = np.random.uniform(-100, 100, size=(self.num_hidden_1, self.num_input))
            self.weight_hidden_hidden = np.random.uniform(-100, 100, size=(self.num_hidden_2, self.num_hidden_1))
            self.weight_hidden_output = np.random.uniform(-100, 100, size=(self.num_output, self.num_hidden_2))
        else:
            self.load_weights()
    ...
    def save_weights(self):
        np.savetxt(f'./weights/input_hidden_{dw}{lvl}.txt',
                   self.weight_input_hidden, fmt='%5.4g', delimiter=',')
        np.savetxt(f'./weights/hidden_hidden_{dw}{lvl}.txt',
                   self.weight_hidden_hidden, fmt='%5.4g', delimiter=',')
        np.savetxt(f'./weights/hidden_output_{dw}{lvl}.txt',
                   self.weight_hidden_output, fmt='%5.4g', delimiter=',')

    def load_weights(self):
        self.weight_input_hidden = np.loadtxt(f'./weights/input_hidden_{dw}{lvl}.txt', dtype=float, delimiter=',')
        self.weight_hidden_hidden = np.loadtxt(f'./weights/hidden_hidden_{dw}{lvl}.txt', dtype=float, delimiter=',')
        self.weight_hidden_output = np.loadtxt(f'./weights/hidden_output_{dw}{lvl}.txt', dtype=float, delimiter=',')
    ...
    def create_mixed_weights(self, net1, net2):
        ...
        self.weight_hidden_hidden = NNet.get_mix_from_arrays(net1.weight_hidden_hidden, net2.weight_hidden_hidden)
