...
class Player(pygame.sprite.Sprite):
    ...
    def click_right(self):
        if self.on_ground:
            ...
            self.direction.x = min(1, self.direction.x + self.air_speed)
        self.facing_right = True
    ...
    def click_left(self):
        if self.on_ground:
            ...
            self.direction.x = max(-1, self.direction.x - self.air_speed)
    ...
    def get_nn_input(self, frame):
        inputs = self.nnet.get_final_value([self.rect.x / screen_width, self.rect.y / screen_height, frame])
    ...
    def create_offspring(p1, p2):
        ...
        new_player_sprite.nnet.create_mixed_weights(p1.sprite.nnet, p2.sprite.nnet)
...
class PlayerCollection:
    ...
    def evolve_population(self, level_map):
        ...
        for player in self.players:
            if player.sprite.is_alive:
                dist = self.bfs(level_map, (int(player.sprite.rect.x/10), int(player.sprite.rect.y/10)))
       ...
       #print("Avg. fitness:", total_fitness/generation_size)
        print("Score", total_fitness/generation_size)
        ...
        while len(new_players) < len(self.players):
            ...
            if idx_to_breed[0] != idx_to_breed[1]:
                new_player = Player.create_offspring(good_players[idx_to_breed[0]], good_players[idx_to_breed[1]])
